...
public class Day17 {
    ...
    static class Node {
        int count, x, y, steps;
        int[] dir;
        public Node(int _count, int _x, int _y, int[] _dir, int _steps) {
            count = _count; x = _x; y = _y; dir = _dir; steps = _steps;
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        // part 1
        final ArrayList<Node> open =new ArrayList<>();
        open.add(new Node(0, 0, 0, dirs[1], 0));
        final HashMap<Key, int[]> closed =new HashMap<>(); // Key -> {count, consec}
        ...
        while (open.size() > 0) {
            ...
            int count = n.count, x = n.x, y = n.y, consec = n.steps;
            int[] dir = n.dir;
            dirLoop:
            for (int[] i : dirs) {
                int X = x + i[0], Y = y + i[1];
                ...
                if (X == mapSize-1 && Y == mapSize-1) {
                    ...
                    for (int j = open.size() - 1; j >= 0; j--) {
                        ...
                        if (openNode.x == X && openNode.y == Y && openNode.count <= newCount && openNode.steps <= newConsec && openNode.dir == i) continue dirLoop;
                    }
